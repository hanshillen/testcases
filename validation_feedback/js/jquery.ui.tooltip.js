...
$.widget( "ui.tooltip", {
	...
	open: function( event ) {
		...
		// if tooltipId data exists, then the tooltip is already open
		if ( !target.length || target.data("tooltipId") ) {
			return;
		}
	...
	_open: function( event, target, content ) {
		...
		if (target.attr( "aria-describedby" )) {
			target.attr( "aria-describedby", target.attr( "aria-describedby" ) + " " + tooltip.attr( "id" ) )
		}
		else {
	...
	close: function( event, force ) {
		...
		var descriptionIds = target.attr( "aria-describedby" ).split(" ");
		if (descriptionIds.length > 1) {
			var tooltipIndex = $.inArray( target.data( "tooltipId" ), descriptionIds );
			if (tooltipIndex != -1) {
				descriptionIds.splice(tooltipIndex, 1)
				target.attr( "aria-describedby", descriptionIds.join( " " ) );
			}
		}
		else {
			target.removeAttr( "aria-describedby" );
		}
		target.removeData( "tooltipId" );
