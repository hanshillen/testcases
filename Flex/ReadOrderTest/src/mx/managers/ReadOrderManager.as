...
package mx.managers
{
    ...
    public class ReadOrderManager extends EventDispatcher
    {
        ...
        private function isEnabledAndVisible(o:DisplayObject):Boolean
        {
            ...
            while (o != formParent)
            {
                if (o is IUIComponent)
                    if (!IUIComponent(o).enabled || !IUIComponent(o))
                        return false;
                /*
        ...
        {
            trace(createIndent() + ">>addFocusables " + o + getTextContent(o));
            _nesting++;
                ...
                && io.parent.tabChildren)
            {
                if (readableObjects.indexOf(o) == -1)
                {
                    ...
                    trace(createIndent(1) + _counter + ": RM added " + o);
                    _counter++;
                }
            }
            ...
            if (o is DisplayObjectContainer && DisplayObjectContainer(o).tabChildren)
            {
                ...
                {
                    trace(createIndent(1) + "using view rawChildren");
                    ...
                    for (i = 0; i < rawChildren.numChildren; i++)
                    {
                        ...
                        catch(error:SecurityError)
                        {
                            ...
                            trace(createIndent(2) + "addFocusables: ignoring security error getting child from rawChildren: " + error);
                            _nesting--;
                        }
                    }
                ...
                {
                    trace(createIndent(1) + "using container's children");
                    // recursively visit and add children of components
                    // we don't do this for containers because we get individual
                    // adds for the individual children
                    for (i = 0; i < doc.numChildren; i++)
                    {
                        ...
                        {
                            // Ignore this child if we can't access it
                            trace(createIndent(2) +  "addFocusables: ignoring security error getting child at document." + error);
                            _nesting--;
                        }
                    }
                }
            ...
            _nesting--;
            trace(createIndent() + "<<addReadables " + o);
        }
        ...
        private function updateReadingOrder(fix:Boolean):void
        {
            ...
            for (var i:uint = 0; i < readableObjects.length; i++) 
            {
                try {
                    ...
                    trace(i + "-" + obj.tabIndex + ": " + obj + getTextContent(obj));
                }
                catch (e:Error) {}
            }
        }
        ...
        private function createIndent(add:int=0):String {
            var indentStr:String = "";
            var loops:int = _nesting + add;
        ...
        private function getTextContent(o:DisplayObject):String
        {
